  ...
  if (sent) {
    return (
      ...
      >
        ...
        <h3 className="text-2xl font-bold text-foreground">{t("partner.success")}</h3>
        <p className="max-w-md leading-relaxed text-muted-foreground">{t("partner.successTextLong")}</p>
        <button
          onClick={() => {
            setValues(initial)
            setSent(false)
    ...
    >
      <div className="flex items-center gap-3 border-b border-border bg-navy px-6 py-5 text-navy-foreground md:px-8">
        <span className="flex size-11 items-center justify-center rounded-xl bg-white/10">
          <Handshake className="size-6 text-amber" />
        </span>
        <div>
          <h2 className="text-lg font-bold">{t("partner.formTitle")}</h2>
          <p className="text-sm text-navy-foreground/70">{t("partner.formSubShort")}</p>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="p-6 md:p-8">
        <div className="grid gap-5 sm:grid-cols-2">
          <Field label={t("partner.company")} required icon={<Building2 className="size-4" />}>
            <input
              type="text"
              required
          ...
          <Field label={t("partner.address")} required icon={<MapPin className="size-4" />}>
            <input
              type="text"
              required
          ...
          <Field label={t("partner.person")} required icon={<User className="size-4" />}>
            <input
              type="text"
              required
              value={values.contact}
              onChange={(e) => update("contact", e.target.value)}
              placeholder={t("partner.person")}
              className="form-input"
            />
          </Field>
          <Field label={t("partner.personPhone")} required icon={<Phone className="size-4" />}>
            <input
              type="tel"
              required
              dir="ltr"
        ...
        <div className="mt-5">
          <Field label={t("partner.email")} required icon={<Mail className="size-4" />}>
            <input
              type="email"
              required
              dir="ltr"
        ...
        <div className="mt-5">
          <Field label={t("partner.messageSubject")} required icon={<MessageSquare className="size-4" />}>
            <textarea
              required
              maxLength={180}
              rows={5}
